...
const Houses = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      alert(t('users.self_delete_error')); // Usamos una clave de error genérica
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          <h1 className="text-3xl font-black text-slate-800 tracking-tight">{t('houses.title')}</h1>
          <p className="text-slate-500 font-medium">
            {t('houses.pagination', { current: currentHouses.length, total: filteredHouses.length })}
          </p>
        </div>
        
        <button 
          onClick={() => { setEditingHouse(null); setFormData({ address: '', price: '', status: 'disponible' }); setIsModalOpen(true); }}
          className="flex items-center gap-2 bg-indigo-600 text-white px-6 py-3 rounded-2xl font-bold shadow-lg shadow-indigo-100 active:scale-95 transition-all"
        >
          <Plus size={20} /> {t('houses.add_btn')}
        </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        <div className="flex bg-white p-1.5 rounded-2xl shadow-sm border border-slate-200 w-full md:w-fit">
          {['all', 'disponible', 'vendido'].map((f) => (
            ...
            >
              {f === 'all' ? t('houses.filter_all') : f === 'disponible' ? t('houses.filter_available') : t('houses.filter_sold')}
            </button>
          ))}
        </div>
            ...
            <div className="p-8">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-end">
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase">{t('houses.table_price')}</p>
                  ...
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${house.status === 'disponible' ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'}`}>
                    {house.status === 'disponible' ? t('houses.filter_available') : t('houses.filter_sold')}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-3xl border border-slate-200 gap-4">
        <p className="text-sm font-bold text-slate-500">
          {t('global.view')} {currentHouses.length > 0 ? startIndex + 1 : 0} {t('global.actions')} {startIndex + currentHouses.length} {t('houses.pagination', { current: currentPage, total: totalPages || 1 })}
        </p>
        <div className="flex items-center gap-2">
          ...
          <span className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl font-bold text-sm">{t('houses.pagination', { current: currentPage, total: totalPages || 1 })}</span>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-indigo-950/40 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full p-8 text-center animate-in zoom-in duration-200">
            ...
            <h2 className="text-xl font-bold text-slate-800 mb-2">{t('houses.delete_title')}</h2>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 relative animate-in zoom-in duration-200">
            ...
            <h2 className="text-2xl font-bold mb-6 text-slate-800 tracking-tight">{editingHouse ? t('houses.modal_edit') : t('houses.modal_add')}</h2>
            <form onSubmit={handleSave} className="space-y-5">
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2 ml-1">{t('houses.address_label')}</label>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2 ml-1">{t('houses.price_label')}</label>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2 ml-1">{t('global.status')}</label>
                <select className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-indigo-500 font-bold" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})}>
                  <option value="disponible">{t('houses.filter_available')}</option>
                  <option value="vendido">{t('houses.filter_sold')}</option>
                </select>
              </div>
              <button type="submit" className="w-full p-4 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-indigo-700 transition-colors">{t('global.save')}</button>
            </form>
          </div>
        </div>
      )}
      
      <InfoDetailCard isOpen={!!selectedHouse} onClose={() => setSelectedHouse(null)} title={t('houses.modal_details')} icon={Building2} data={{"Dirección": selectedHouse?.address, [t('houses.table_price')]: `$${Number(selectedHouse?.price).toLocaleString()}`, [t('global.status')]: selectedHouse?.status?.toUpperCase()}} />
    </div>
  );
};
